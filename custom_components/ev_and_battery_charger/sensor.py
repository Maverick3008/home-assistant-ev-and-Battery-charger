...
class EVAndBatteryChargerCalculator:
    ...
    def _get_target_soc(self) -> float | None:
        ...
        legacy_target_entity = str(
            self.config.get(CONF_TARGET_SOC_ENTITY, "") or ""
        ).strip()
    ...
    def _calendar_event_signature(
        ...
        event_end_text = (
            event_end.isoformat() if isinstance(event_end, datetime) else ""
        )
        return (
            f"{calendar_entity}|{event_start.isoformat()}|"
            f"{event_end_text}|{event_title}"
        )
    ...
    def _sync_calendar_target_soc(
        ...
        This prevents the target from falling back to 80% in the middle of an
        overnight charge merely because the event changed from "tomorrow" to
        "today".
        """
        ...
        if event_signature is None:
            ...
            # Once the completed event disappears from the calendar entity, a
            # later event may become eligible when it reaches "tomorrow".
        ...
        # Keep the already activated event at 100%, including after midnight.
        if event_signature == active_event:
            self._set_runtime_target_soc(CALENDAR_AUTO_TARGET_SOC)
            return

        # The calendar now exposes another event. If a previous uncompleted
        # event had activated 100%, release that automatic target first.
        if active_event is not None:
            entry_data["calendar_target_event"] = None
            self._set_runtime_target_soc(CALENDAR_RESET_TARGET_SOC)
    ...
    def _get_calendar_ready_by(
        self, calendar_entity: str, now: datetime
    ...
    def _get_daily_ready_by(
        self, target_time_value: str | time, now: datetime
    ) -> datetime:
        """Return the next daily ready-by datetime."""

        target_time = self._parse_time(target_time_value)
        ready_by = datetime.combine(now.date(), target_time).replace(
            tzinfo=now.tzinfo
        )
        if ready_by <= now:
    ...
    def calculate(self) -> ChargeCalculation | None:
        ...
        battery_size_kwh = float(
            config.get(CONF_BATTERY_SIZE_KWH, DEFAULT_BATTERY_SIZE_KWH)
        )
        charge_power_kw = float(
            config.get(CONF_CHARGE_POWER_KW, DEFAULT_CHARGE_POWER_KW)
        )
        efficiency = float(config.get(CONF_EFFICIENCY, DEFAULT_EFFICIENCY))
        buffer_minutes = int(
            float(config.get(CONF_BUFFER_MINUTES, DEFAULT_BUFFER_MINUTES))
        )
        target_time_value = config.get(CONF_TARGET_TIME, DEFAULT_TARGET_TIME)
        target_source_priority = str(
            config.get(
                CONF_TARGET_SOURCE_PRIORITY, DEFAULT_TARGET_SOURCE_PRIORITY
            )
        ).strip()
        ...
        calendar_ready_by, calendar_details = self._get_calendar_ready_by(
            calendar_entity, now
        )
        ...
        if target_source_priority == TARGET_PRIORITY_DAILY_TIME_FIRST:
            ...
            if (
                calendar_ready_by is not None
                and calendar_ready_by < daily_ready_by
            ):
        ...
        if (
            soc_diff <= 0
            or battery_size_kwh <= 0
            or charge_power_kw <= 0
            or efficiency <= 0
        ):
            ...
            energy_needed_kwh = (
                (soc_diff / 100) * battery_size_kwh / efficiency
            )
            exact_duration_minutes = (
                energy_needed_kwh / charge_power_kw
            ) * 60
        ...
        configured_full_charge_extra_minutes = max(
            configured_full_charge_extra_minutes, 0
        )
        ...
        if self._completed_charge_window_signature == charge_window_signature:
            ...
            locked_duration_minutes = (
                self._locked_duration_minutes or duration_minutes
            )
            locked_exact_duration_minutes = (
                self._locked_exact_duration_minutes or exact_duration_minutes
            )
            locked_energy_needed_kwh = (
                self._locked_energy_needed_kwh or energy_needed_kwh
            )
            ...
            self._locked_charge_finished_at = now + timedelta(
        ...
        minutes_until_start = round(
            (planned_start - now).total_seconds() / 60
        )
...
class EVAndBatteryChargerSensor(SensorEntity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        legacy_target_entity = str(
            config.get(CONF_TARGET_SOC_ENTITY, "") or ""
        ).strip()
        if legacy_target_entity:
    ...
    def extra_state_attributes(self) -> dict[str, Any] | None:
        ...
        return {
            ...
            "duration_minutes_exact": round(
                calculation.exact_duration_minutes, 2
            ),
            ...
            "locked_charge_started_at": (
                calculation.locked_charge_started_at.isoformat()
                if calculation.locked_charge_started_at
                else None
            ),
            "locked_charge_finished_at": (
                calculation.locked_charge_finished_at.isoformat()
                if calculation.locked_charge_finished_at
                else None
            ),
            ...
            "legacy_target_soc_entity": self.calculator.config.get(
                CONF_TARGET_SOC_ENTITY
            ),
            "calendar_entity": calculation.calendar_entity,
            "calendar_event_title": calculation.calendar_event_title,
            "calendar_event_start": (
                calculation.calendar_event_start.isoformat()
                if calculation.calendar_event_start
                else None
            ),
            "calendar_event_end": (
                calculation.calendar_event_end.isoformat()
                if calculation.calendar_event_end
                else None
            ),
